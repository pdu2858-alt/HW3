...
def main():
    ...
    # on-the-fly eval options
    onfly_sample_n = st.sidebar.number_input("Eval sample size (on-the-fly)", min_value=50, max_value=5000, value=500, step=50)
    onfly_auto = st.sidebar.checkbox("Auto-run on-the-fly eval when saved eval missing", value=False)
    ...
    with tab_model:
        ...
        if not eval_data:
            ...
            if selected is not None and selected.exists() and df is not None and label_col in df.columns:
                # allow user to trigger on-the-fly evaluation to avoid auto-running on page load
                run_eval = False
                if onfly_auto:
                    run_eval = True
                if st.button("Run on-the-fly evaluation (sample)"):
                    run_eval = True

                if run_eval:
                    st.info("Running on-the-fly evaluation using the selected model and dataset sample. This may take a moment.")
                    try:
                        m = load_model(selected)
                        texts = df[text_col].fillna("").astype(str).tolist()
                        sample_n = min(len(texts), int(onfly_sample_n))
